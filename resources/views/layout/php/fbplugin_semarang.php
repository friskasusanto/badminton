<div class="news-swipe-place">
	<?php 
		...
		$str = file_get_contents('http://dev.cplusco.com/assets/facebook/pluginFb.php?pageid=1483370808565658&type=JSON&limit=10&since='.$twoMonthEarlier);
	...
	$(function(){
		...
		$('.news-swipe-place').on('swipe', function(event, slick, direction){
			...
			if(is_nearly_end < 1 && window.feedFetch == false){
				...
				spinning('news-load');
				var link = 'http://dev.cplusco.com/assets/facebook/pluginFb.php?pageid=1483370808565658&type=JSON&until='+until+'&since=<?php echo $twoMonthEarlier ?>';
				$.get(link)
				.done(function(data){
					window.feedFetch = false
					$('.news-load').remove();
